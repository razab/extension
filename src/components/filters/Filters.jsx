...
export const Filters = () => {
  useEffect(() => {
    console.log("TESTEXT: popup loaded ...");
    onPageLoad();
  })
  return (
    <>
      <div id="content">
        <section>
          ...
          <div className="hint">Exact full string matching</div>
          <div id="countries" className="inputList"></div>
          <form id="add_country_form" onSubmit={formSubmit} className="addFilterForm">
            <input name="name" placeholder="Add country to filter out"/>
            <button className="formButton"></button>
          </form>
        </section>
        <section className="last">
          <h3>Title filter</h3>
          <div className="hint">Title's part matching to listed words/phrases</div>
          <div id="titles" className="inputList"></div>
          <form id="add_title_form" onSubmit={formSubmit} className="addFilterForm">
            <input name="name" placeholder="Add title filter"/>
            <button className="formButton"></button>
          </form>
        </section>
      </div>
      <section>
        <button id="cleanHistory" onClick={cleanHistory}>Clean history of viewed jobs <i>(all viewed jobs will become white again)</i></button>
      </section>
    </>
  )
}
...
const STORAGE_PROP_NAME_FOR_CHECKED_JOBS = '_UpworkCleaner_checkedJobs',
  ...
  TITLES_ID = 'titles',
  JOBS_FEED_URL = ['https://www.upwork.com/nx/find-work/*', 'https://www.upwork.com/nx/find-work/*'];
...
function buildFilters(filters) {
  console.log(`TESTEXT: buildFilters `, filters)
  const callback = function (allFilters = {}) {
    console.log(`TESTEXT: buildFilters callback`, allFilters)
